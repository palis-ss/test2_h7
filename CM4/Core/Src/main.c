...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  {		
    ...
    /* USER CODE BEGIN 3 */		
		if(!HAL_HSEM_IsSemTaken(HSEM_CNTR))  // wait for signal from Core1
		{			
			if(cnt >= 5)
			{
				cnt = 0;
				//HAL_GPIO_TogglePin(LD2_GPIO_Port, LD2_Pin);
				HAL_GPIO_TogglePin(LD3_GPIO_Port, LD3_Pin);  // LD3 defined in Core1
				
				EnterCriticalSection();
				counter++;
				mydouble = (double)counter;
				LeaveCriticalSection();
				
			}
			cnt++;
			HAL_Delay(1);  // give Sem a chance to lock in Core1 before entering next loop
		}
  }
  /* USER CODE END 3 */
}
